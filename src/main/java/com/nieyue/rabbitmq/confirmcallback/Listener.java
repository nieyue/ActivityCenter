...
public class Listener {
	...
	String projectName;
	     ...
	     * @throws IOException
	     */
		    @RabbitListener(queues="${myPugin.rabbitmq.ACTIVITYWEBREAD_DIRECT_QUEUE}") 
		    public void ActivityWebRead(Channel channel, DataRabbitmqDTO dataRabbitmqDTO,Message message) throws IOException   {
		           try {
		        	  /**
		        	   * 判断是否存在
		        	   */
		        	 //如果活动不予统计  
		       		Activity inActivity = activityService.loadSmallActivity(dataRabbitmqDTO.getActivityId());
		       		if(inActivity==null||inActivity.equals("")){
		        	   ...
		        	   BoundSetOperations<String, String> bsodatips = stringRedisTemplate.boundSetOps(projectName+"ActivityId"+dataRabbitmqDTO.getActivityId()+"Data"+DateUtil.getImgDir()+"Ips");
		        	   int isAddIp=0;//默认没增加
		        	   ...
		        	   //统计当日当前人的当前活动每日ip
		        	   BoundSetOperations<String, String> bsodataips = stringRedisTemplate.boundSetOps(projectName+"AcountId"+readinacount.getAcountId()+"ActivityId"+dataRabbitmqDTO.getActivityId()+"Data"+DateUtil.getImgDir()+"Ips");
				        	   if(isAddIp==1){
		   			...
		   			realdata.setActivityId(dataRabbitmqDTO.getActivityId());
		   			...
		   			realdailydata.setActivityId(dataRabbitmqDTO.getActivityId());
		        	   ...
		        	   */
		        	   //当前活动
		        	   Activity activity = activityService.loadSmallActivity(dataRabbitmqDTO.getActivityId());
		        	   activity.setReadingNumber(activity.getReadingNumber()+isAddIp);
		        	   activity.setPvs(activity.getPvs()+1);
		        	   activity.setUvs(activity.getUvs()+dataRabbitmqDTO.getUv());
		        	   activity.setIps(activity.getIps()+isAddIp);
		        	   activityService.updateActivity(activity);//更新活动数据
		        	  channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
				} catch (Exception e) {
